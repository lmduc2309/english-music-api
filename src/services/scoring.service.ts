import { SCORING_WEIGHTS, PASS_THRESHOLD, XP_CONFIG } from '../config/constants';
import { IWordScore } from '../models/PracticeAttempt';
...
export class ScoringService {
  /**
   * Main scoring method - calculates pitch, duration, pronunciation scores
   * and returns weighted overall with pass/fail determination
   */
  static calculateScore(input: ScoringInput, currentStreak: number = 0): ScoringResult {
    ...
    const overall = Math.round(
      pitchScore * SCORING_WEIGHTS.pitch +
      durationScore * SCORING_WEIGHTS.duration +
      pronunciationScore * SCORING_WEIGHTS.pronunciation
    );
  ...
  /** Compare pitch contours using semitone distance */
  static calculatePitchScore(userPitch: number[], refPitch: number[]): number {
    ...
    for (let i = 0; i < len; i++) {
      ...
      if (r > 0 && u > 0) {
        const semitoneDiff = Math.abs(12 * Math.log2(u / r));
        total += Math.max(0, 1 - semitoneDiff / 6);
      }
    }
    return (total / len) * 100;
  }

  /** Duration accuracy with ±20% tolerance for full score */
  static calculateDurationScore(userDuration: number, refDuration: number): number {
  ...
  /** Word-level pronunciation scoring using Levenshtein similarity */
  static calculatePronunciationScore(spokenWords: string[], expectedWords: string[]): { pronunciationScore: number; wordScores: IWordScore[] } {
  ...
  /** Levenshtein-based word similarity (0-1) */
  ...
  static resampleArray(arr: number[], targetLength: number): number[] {
  ...
  static generateFeedback(pitch: number, duration: number, pronunciation: number, wordScores: IWordScore[]): string[] {
    const fb: string[] = [];
    ...
    if (duration >= 90) fb.push('\u23f1\ufe0f Perfect timing and rhythm!');
    else if (duration >= 70) fb.push('\u23f1\ufe0f Good rhythm! Try to match the pace more closely.');
    else fb.push('\u23f1\ufe0f Focus on the rhythm. Try clapping the beat before singing.');

    if (pronunciation >= 90) fb.push('\ud83d\udde3\ufe0f Excellent pronunciation!');
    else if (pronunciation >= 70) fb.push('\ud83d\udde3\ufe0f Good pronunciation! A few words need work.');
    else fb.push('\ud83d\udde3\ufe0f Listen carefully to each word and try again.');

    const incorrect = wordScores.filter((w) => !w.correct);
    if (incorrect.length > 0 && incorrect.length <= 3) {
      fb.push(`\ud83d\udcdd Pay attention to: ${incorrect.map((w) => `"${w.word}"`).join(', ')}`);
    } else if (incorrect.length > 3) {
      fb.push(`\ud83d\udcdd ${incorrect.length} words need improvement. Listen and try again!`);
    }
    return fb;
  }

  /** Calculate XP with streak multiplier */
  static calculateXP(overallScore: number, currentStreak: number): number {
    ...
    const mult = Math.min(1 + currentStreak * 0.1, XP_CONFIG.maxStreakMultiplier);
